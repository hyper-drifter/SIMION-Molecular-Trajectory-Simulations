...
-- ************************************************************
  ...
  mu = 1. * debye * 1000 *6.242*10^18						-- permanent dipole moment [Debye] 4.5 for Benzonitrile. Multiplied by 1000 to convert to mm  and 6.242*10^18 to convert joules to eV.
...
pa:refine{convergence=10^-7}								-- , skipped_point = true

pa:fast_adjust{[1]=-quadVoltage, [2]=0, [3]=quadVoltage, [4]=0}



--make PA of electric field magnitude values
pa1 = simion.pas:open()
pa1:load 'firstStage.pa0'
--eMagGrid = io.open('electricFieldMag.tsv','w+')
for xg, yg, zg in pa1:points() do
      if not pa1:electrode(xg,yg,zg) then
            Ex,Ey,Ez = pa:field_vc(xg,yg,zg)
            Emag = math.sqrt(Ex^2+Ey^2+Ez^2)*scaleFactor

            --[[creates tsv file of electric field magnitudes on the diagonal.
            pa1:potential(xg,yg,zg,Emag)
            if zg == 0 and xg==299-yg then
                  eMagGrid:write(xg..'\t'..yg..'\t'..Emag..'\n')
            end]]--
      end
end
--eMagGrid:close()
pa1:save('firstStageEField.pa0')
...
for xg, yg, zg in pa2:points() do
	if not pa2:electrode(xg,yg,zg) then
		...
		pa2:potential(xg,yg,zg,W)
	end
end
pa2:save('firstStageStark.pa0')
...
pa:save('secondStage.pa#')
pa:refine{ convergence=10^-7}								-- , skipped_point = true

pa:fast_adjust{[1]=0, [2]=-quadVoltage, [3]=0, [4]=quadVoltage}



--make 2nd PA of electric field magnitude values
pa1 = simion.pas:open()
pa1:load 'firstStage.pa0'
for xg, yg, zg in pa1:points() do
      if not pa1:electrode(xg,yg,zg) then
...
--second stark potential PA
pa3 = simion.pas:open()
pa3:load 'secondStage.pa0'

--[[
      Authors: Taylor Grubbs, adapted from Demitri Balabanov's original code.
      Purpose: This code builds the electrode geometries of the proposed Stark Decelerator microchip.
--]]
...
quadScale = 1 --variable used to scale the entire simulation up or down. Values greater than 1 decrease the size while values between 0 and 1 increase the size. I think we will only want to scale down the simulation and I like whole numbers so that's why this seems a little backwards.

local quadVoltage = 200/quadScale --  voltage magnitude of quadrupoles. ENTER THE EXACT VALUE THAT YOU WILL USE IN THE REAL WORLD EXPERIMENT NO SCALING NEEDED

scaleFactor = 6000*quadScale --reduces PA size parameters by this factor; 100 converts each GU into 10 um which is needed for the Max-Planck experimental setup (want 1mm radius rods)
--voltage and other potential calculations will not be affected by this value.

local radius = 1/quadScale --radius of rods is 1 mm = 100 10^-5 m
...
--This part creates the electrodes
pa:fill{function(xg,yg,zg)

      if yg >= .0499/quadScale and xg >= .045/quadScale then

            return 1,true

      elseif yg >= .0499/quadScale and xg <= .005/quadScale then

            return 2,true

      elseif yg <= .0001/quadScale and xg <= .005/quadScale then

            return 3,true

      elseif yg <= .0001/quadScale and xg >= .045/quadScale then
...
pa2:save('1stStageEfield.pa0')

--- make PA containing Stark Potential ---

pa3 = simion.pas:open()
pa3:load '1stStage.pa0'
...
for xg, yg, zg in pa3:points() do
	if not pa3:electrode(xg,yg,zg) then
		...
		W = starkLinearShift(mu,Emag)	-- eV

		if xg==150 and yg==50 then
...
pa2:save('2ndStageEfield.pa0')

--- make PA containing Stark Potential ---

pa3 = simion.pas:open()
pa3:load '2ndStage.pa0'
